/*
Champ input tri photo:
Mettre l'item selectionner dans le champs lors du click
*/
  ...
  photographers.forEach(element => {
    ...
    const profilTemplate = `
      <section aria-label="Photographer Profil" class="profil-container">
        ...
        <p >${element.tags.map(tag => `<a  id="cursorAjout" href="../../home.html?id=${tag}" class='tags'>#${tag}</a>`).join(" ")}</p>
        <button id="form">Contactez-moi</button>
        <div class="photoBox">
            <img src="../../img/Photographers ID Photos/${element.portrait}" alt="photo de ${element.name}">
        </div>
      </section>
    `
    document.querySelector(".dropBtn").innerHTML = "Popularité"
...
function photographerWork(media){
  ...
  homeElt = window.location.search.split('id=')[1];  
    ...
    if(homeElt == element.photographerId){
      ...
      domDiv.appendChild(newDiv);
      if ( 'image' in element) {currentPhotographerPhotos.push(element.image), nomPhoto.push(element.photo), currentRepo.push(element.repo)}
...
function videoOrimage(image, video, element) {
  if ('image' in element){
    return ` <img class="photos" aria-label="photo ${element.nomPhoto}" src="../../img/${element.repo}/${image}"> `
  }
  else if ('video' in element){
    return ` <iframe src="../../img/${element.repo}/${video}" width="285px" height="255px" controls=0></iframe>`
  }
}
...
trierParButtons.forEach((btn, index) => btn.addEventListener('click', () => {
  if( index == 1) {
    ...
    document.querySelector("#photographers").innerHTML = "";
    document.querySelector(".dropBtn").innerHTML = "Date"
    likesTable = [];
    currentPhotographerPhotos = [];
    photographerWork(modifiedArray);
    openLightBox(JsonDATA)
    ...
    modifiedArray = JsonDATA.media.sort((a, b) => {
      if (a.photo.toLowerCase() < b.photo.toLowerCase()) {
        { return -1; }
      }else if (a.photo.toLowerCase() > b.photo.toLowerCase()) {
        { return 1; }
      }
    })
        document.querySelector("#photographers").innerHTML = "";
        document.querySelector(".dropBtn").innerHTML = "Titre"
        likesTable = [];
        currentPhotographerPhotos = [];
        photographerWork(modifiedArray);
        openLightBox(JsonDATA)
...
function openLightBox() {
  const getPhotos = Array.from(document.getElementsByClassName('photo'));
    getPhotos.forEach((photo, index) => photo.addEventListener("click", () => {
      ...
      const repo = currentRepo[1];
      
      lightboxcontainer.style.display = 'block';
      currentLigthboxIndex = index;   
      photographersphotos.innerHTML = `<img src="../../img/${repo}/${src}" />`;
